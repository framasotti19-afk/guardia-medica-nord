...
> **APP DEFINITIVA (voci 98-106):** hosting statico `docs/` su Pages; dati in `public.app_state` (blob JSON `store`, RLS + login email/password); AI via Edge Function Supabase `chiedi-ai` (chiave `ANTHROPIC_API_KEY` solo nel secret). URL: `https://framasotti19-afk.github.io/guardia-medica-nord/`. Supabase project `laerjxdpgipgrrinffim`. **`buildSheetModel` è la fonte condivisa Excel↔PDF** (voce 98); Excel byte-ref `2c002077` **SUPERATO dalla voce 108** (l'Excel dipende ora dallo schema nuovo — nessun hash fisso di riferimento; export code intatto); PDF **vettoriale a mano** (`buildPdfBytes`, voce 103) — si allinea da solo perché legge lo stesso `buildSheetModel`. **NON fatto per scelta (un solo coordinatore):** migrazione dati, conflitti multi-utente, coda offline, refresh-token dedicato. Se arriva un 2° coordinatore → rivalutare SOLO i conflitti (riga condivisa, last-write-vince).

> **MOTORE:** la regola Claut/Anduins-diurno è **CAMBIATA** (**voce 108**: Claut/Anduins mai fisiche, diurno come il notturno — cambio DELIBERATO del motore) → **l'era byte-identica è CHIUSA**. Ultimo hash della vecchia catena `89f9bf3e` (v96); il metodo di quella catena **NON è riproducibile dal repo** (harness perso in `scratchpad/`). Nuovo riferimento **riproducibile**: `git rev-parse HEAD:engine_test.mjs` = **`75e6a13f`**, `sha256(regione-motore)[:8]` = **`932366e1`**. Il guard VERO ora è: **unit 66/66** + **sim 100k = 0 violazioni su 34.779.900 check** (+ banco mirato COMBO/CHIUSURA **14/14**: il notturno non è toccato). ⚠️ **RED-LINE MOTORE:** ogni modifica al motore è un cambio deliberato a sé, con sim 100k piena e testa fresca, MAI impilato su altro lavoro a fine nottata. NB (voce 96): la "cascata di 14.900 slot" che aveva bloccato questa modifica era un **artefatto di misura** (medici seedati su un solo motore) — ri-misurato: **0 diff fisici su 260.000 turni**. Vedi la lezione "§ IL METRO PRIMA DELL'OGGETTO — anche quando il metro sei TU" in CONTEXT.

> **VERIFICA EXPORT — il sostituto dell'hash `2c002077` (voce 108).** L'hash fisso dell'Excel NON vale più: l'export code è intatto ma l'Excel dipende dallo schema, che dipende dai dati → nessun byte-ref fisso. Il metro nuovo è lo script **`verifica_mese.mjs`** (controllo di invarianti semantici, vale per QUALSIASI mese). **Comando:** `node verifica_mese.mjs <store.json> <YYYY-M>` — es. `node verifica_mese.mjs store.json 2026-7` (agosto); senza `YYYY-M` controlla tutti i mesi nello store; senza argomenti fa un **autotest** (mese valido + mese rotto). `<store.json>` = il blob che l'app salva su Supabase (`app_state.data`); `store[mKey].schema` è lo schema del mese. **Verifica 5 invarianti:** (1) Claut/Anduins **mai fisiche**; (2) **"servizio non attivo"** = Claut/Anduins vuote nei notturni dei giorni con diurno; (3) nessun **doppione fisico**; (4) copertura a distanza legittima (coprente fisicamente presente + vincolo territoriale Claut←Maniago, Anduins←Spilimbergo/Meduno); (5) **catena fisica** (Meduno fisico ⟹ entrambe le CDC presidiate). Sono le stesse proprietà della sim 100k, applicate al mese VERO. `✅ tutti gli invarianti rispettati` = Excel/PDF corretti per costruzione (il codice export è intatto, disegna lo schema); altrimenti elenca giorno/turno/motivo. **Come mi fido dell'export senza hash:** motore provato per ogni input (unit 66/66 + sim 0 violazioni) × export fedele (mirror byte-verificato) × questo controllo sul mese specifico.

> **MERGE A MAIN — checklist Pages (per quando si unirà il branch al ramo principale).** Oggi Pages pubblica in modalità *Deploy from a branch*, sorgente = branch **`claude/new-session-tufavl`**, cartella **`/docs`**. Il rischio non è il codice ma la CONFIGURAZIONE della sorgente. Ordine (l'ordine è ciò che evita la rottura): **(1)** unisci `claude/new-session-tufavl` nel ramo principale (main/master) — così `/docs` completo (app.jsx, index.html, vendor/, manifest, icone) finisce su main; **(2)** verifica che `main/docs` sia completo PRIMA di toccare Pages; **(3)** solo ora Settings → Pages → *Deploy from a branch* → Branch **`main`**, cartella **`/docs`** → Save (parte un nuovo build); **(4)** aspetta il build verde (tab Actions) + hard refresh del sito; **(5)** solo DOPO che main funziona, eventualmente cancella il branch vecchio.
> ⚠️ **TRAPPOLA NUMERO UNO: NON cancellare `claude/new-session-tufavl` finché la sorgente di Pages punta ancora lì.** Se cancelli il branch-sorgente prima di spostare la sorgente su main, Pages perde la fonte e **il sito si rompe**. Sposta la sorgente PRIMA, cancella il branch DOPO.
> **Cosa NON cambia (e perché il sito non si rompe):** l'URL resta `https://framasotti19-afk.github.io/guardia-medica-nord/` (project site: dipende da utente+repo, non dal branch) → **CORS della Edge Function e config Supabase invariati** (login/dati/AI continuano identici); i percorsi in `index.html` sono **relativi** (`./app.jsx`, `./vendor/…`, `./manifest.webmanifest`) → nulla da riscrivere. NB: conferma prima se il ramo di default è `main` o `master`.
...
- **107 — sigle sotto i pallini (tab Disponibilità).** Display puro: una riga per pallino verde/blu, pari merito unito da "/", rientro a blocco. `dati.dispo`/motore/Excel intatti.
- **108 — diurno come il notturno: Claut/Anduins mai fisiche (NUOVA REGOLA AZIENDALE).** Motore CAMBIATO deliberatamente → **fine era `89f9bf3e`** (nuovo rif riproducibile `git rev-parse HEAD:engine_test.mjs` = `75e6a13f`). Fase A: fix per SOTTRAZIONE (stesso `risolviBlu` del notturno, `sediFisiche=[0,1,2]`, rimosso blocco "4° medico" ex voce 32); 7 test riscritti; **sim 100k = 0 violazioni**; banco COMBO/CHIUSURA **14/14**. Fase B: prompt `sys` allineato. Fase C (solo verifica): export code intatto → **Excel `2c002077` SUPERATO** (dipende dallo schema); **cascata** di redistribuzione nomi attesa/valida (dipende dal mese); **"servizio non attivo" INVARIATO**; **PDF si allinea da solo** (stesso `buildSheetModel`). Deploy Pages OK (`62922ea`, `90bdb5a`).
