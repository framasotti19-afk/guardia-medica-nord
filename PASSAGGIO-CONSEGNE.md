...
**STATO:** voci fino a **95** su `claude/new-session-tufavl`. Questo file È ORA NEL REPO (committato in voce 91).
...
> ### ⚠️ LA TASSA DELL'OPZIONE B — leggi prima di toccare qualunque schermata
> La regola **"Claut/Anduins chiuse nei notturni di sab/dom/festivi/prefestivi"** NON vive nel motore — **il motore le copre ancora** (scelta deliberata voce 94: portarla nel motore spostava **14.900 slot, 6.006 sui feriali**; causa: in §3.9 `!target.slots.includes(o.id)` conta la a-distanza come "già nel turno").
> **Vive in CINQUE superfici display** (tutte in COMPONENTE): 1. griglia schema · 2. export celle · 3. export note `notaSlot` · 4. `avvisiUI` · 5. dialogo disponibilità + pillola griglia inserimento. **Predicato unico:** `g.turni.some(x=>x.id==="G")` — se cambia in un posto, cambia in tutti e cinque.
> 🚨 **Ogni schermata nuova deve ricordarsene: il codice non lo dice, i test non lo segnalano** (il 13/7 tre superfici dimenticate trovate a occhio, voce 95). Esposti: **EXPORT PDF** (la più a rischio), (d) pin-inerte, ogni vista futura.
> **Via d'uscita = versione A** (PENDING §7): regola nel motore, un posto solo; **e i cinque filtri vanno RIMOSSI** (la rimozione è PARTE del design della versione A — misurare quante righe si tolgono = quanto la regola sparsa costava; come il correttivo iniquità voce 88 e `giorniTra`, codice morto che sembra vivo). 🔔 **Se salta fuori una SESTA superficie, smetti di rattoppare e fai la versione A.**
