...
- Sezione `STILE DI RISPOSTA E LIMITI` nel prompt: massimo 8 azioni per risposta (storico: 3-4 → 3 → 8), "spiegazione" limitata a una frase breve, vietato citare/ripetere per esteso il testo incollato dall'utente — tutto per restare ampiamente dentro il budget di token ed evitare risposte troncate a metà JSON. Se l'utente chiede più modifiche di quante ne stiano in un round, l'AI ne esegue solo le prime 8, indica nella "spiegazione" solo il conteggio, e imposta il campo strutturato opzionale `"altreAzioniRestanti":true` nella risposta JSON di tipo "modifiche" (default false/omesso). Il fallback dopo una risposta troncata (vedi sotto) resta più conservativo, a 2 azioni, per non rischiare un nuovo troncamento.
