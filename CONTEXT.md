...
```bash
node test_email_generator.mjs                  # genera test_email_corpus.json (1340 casi)
...
- **Medico senza incarico: disponibilità ordinarie bloccate senza un numero di guardie mensili dichiarato**: nuova sottosezione in testa a `INTERPRETAZIONE EMAIL DISPONIBILITÀ` (prima di "SEDI FISICHE — PRIMA SCELTA", quindi valutata prima di ogni altra regola della sezione). Se un medico con categoria "Senza inc." dichiara sedi/giorni/turni ma non ha mai indicato, né in questo messaggio né in uno precedente della stessa conversazione, un numero complessivo di guardie desiderate per il mese, l'AI non deve inserire NESSUNA disponibilità per lui in quel round — solo l'avviso "🔴 ATTENZIONE: [nome] è senza incarico e non ha indicato il numero massimo di guardie mensili — contattare il medico e reinserire l'email con il numero specificato." Se il numero è presente (in questo o in un round precedente, coerente con `REGOLA GENERALE: MEMORIA TRA ROUND`), le disponibilità si inseriscono normalmente. Riguarda solo le disponibilità ordinarie dei senza incarico — non tocca MMG/PLS, recupero ore o turni extra (già coperti dalle rispettive regole). Nessun nuovo campo in `stato` o azione JSON: il numero stesso non viene salvato da nessuna parte, serve solo come condizione di sblocco letta dal testo della conversazione. La regola include un promemoria esplicito a eseguire il controllo silenziosamente (nessun ragionamento visibile in chat), per lo stesso motivo del bug corretto nella sezione CALENDARIO PERPETUO poco sopra. Copertura di test: due nuove categorie in `test_email_generator.mjs`, `senza_incarico_no_numero_guardie` (30 casi, nessun numero dichiarato → avviso, nessuna azione) e `senza_incarico_con_numero_guardie` (30 casi, numero dichiarato nella stessa email → inserimento normale).
