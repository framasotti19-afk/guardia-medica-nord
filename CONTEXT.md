...
**Punto 3 — Aggiustamento mensile del monte ore (bilanciamento turni annui, DET24 e DET12/DET12ASAP).** Il monte ore BASE resta sempre quello dichiarato in `CAT_INFO` (invariato: DET24=104h, DET12/DET12ASAP=52h), ma viene aggiustato di ±8h in mesi specifici PRIMA di calcolare `debitoOrdinarioIniziale` — e quindi anche il tetto automatico di distribuzione (`Math.round(debito/12)`, punto 2 sopra) — così da bilanciare il numero di turni sull'intero anno invece di lasciarlo fisso ogni mese.

- Tabella `AGGIUSTAMENTO_MESE_ORE` (per categoria, mesi 0-based come `MESI_IT`): `DET24: { mesi: [1,3,8,10], delta: -8 }` (Febbraio, Aprile, Settembre, Novembre: 104h→96h, 9→8 turni impliciti); `DET12ASAP` e `DET12: { mesi: [2,4,7,11], delta: 8 }` (Marzo, Maggio, Agosto, Dicembre: 52h→60h, 4→5 turni impliciti). Negli altri 8 mesi dell'anno il monte ore resta quello base (104h/9 per DET24, 52h/4 per DET12/DET12ASAP).
- Totali annui risultanti: **DET24 = 104 turni/anno** (8 mesi da 9 + 4 mesi da 8); **DET12/DET12ASAP = 52 turni/anno** (8 mesi da 4 + 4 mesi da 5) — stesso numero di turni annuo di prima, solo ridistribuito diversamente tra i mesi.
- **INDET e DET36 non hanno mai aggiustamento**: monte ore fisso in ogni mese (96h/8 turni per INDET, 156h/13 per DET36), nessuna voce in `AGGIUSTAMENTO_MESE_ORE`.
...
27. **Aggiustamento mensile del monte ore (bilanciamento turni annui)** — DET24 perde 8h (104h→96h, 9→8 turni impliciti) a Febbraio/Aprile/Settembre/Novembre; DET12 e DET12ASAP guadagnano 8h (52h→60h, 4→5 turni impliciti) a Marzo/Maggio/Agosto/Dicembre; compensato sugli altri 8 mesi (104 turni/anno per DET24, 52 per DET12/DET12ASAP). INDET e DET36 invariati ogni mese. Tocca solo il valore iniziale del debito (`debitoOrdinarioIniziale`, tabella `AGGIUSTAMENTO_MESE_ORE`), nessuna modifica a gerarchia/conflitti/assegnazione (§3.11 punto 3)
