...
- Sezione `STILE DI RISPOSTA E LIMITI` nel prompt: massimo 3 azioni per risposta (ridotto da 3-4), "spiegazione" limitata a una frase breve, vietato citare/ripetere per esteso il testo incollato dall'utente — tutto per restare ampiamente dentro il budget di token ed evitare risposte troncate a metà JSON. Se l'utente chiede più modifiche di quante ne stiano in un round, l'AI ne esegue solo le prime 3, indica nella "spiegazione" solo il conteggio, e imposta il campo strutturato opzionale `"altreAzioniRestanti":true` nella risposta JSON di tipo "modifiche" (default false/omesso).
