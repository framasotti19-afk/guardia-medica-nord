...
suite.test("turno EXTRA (MMG) con preferito non assegnato genera l'avviso di sede come un ordinario", () => {
  // L'MMG compete come un turno ordinario (§10 voce 55): il ★ è valutato sulla sede esatta, con lo
  // STESSO avviso di un ordinario. (Le altre sedi non coperte generano anche un avviso "SCOPERTE": qui
  // verifichiamo solo che ci sia l'avviso del preferito non ottenuto.)
  const g = GIORNI_FERIALI_SEMPLICI[0];
  ...
  const avvisoPref = avvisi.filter((a) => a.includes("PRESSACCO") && a.includes("preferita"));
  suite.eq(avvisoPref.length, 1, "esattamente un avviso sul preferito (sede) di PRESSACCO non ottenuto");
});
