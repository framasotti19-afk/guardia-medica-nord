...
function applicaDispoSet(a, anno, mese, extras) {
  ...
  slots.forEach(({ giorno, turno }) => {
    nd[slotKey(anno, mese, giorno, turno)] = { verde: [...entry.verde], verdeLiv: { ...entry.verdeLiv }, blu: [...entry.blu], bluLiv: { ...entry.bluLiv }, no: false };
  });
  return { nd, errori };
}
...
function applicaDispoAggiungiLista(a, anno, mese, extras, slotsIndip) {
  ...
  for (const { giorno, turno } of slots) {
    const aAgg = { sedi: a.sedi, blu: a.blu, sedi_liv: a.sedi_liv, blu_liv: a.blu_liv };
...
// Set di sedi/livelli variegati da provare su ogni ambito
const SPEC = [
  { sedi: ["Maniago"] },
  { sedi: ["Maniago", "Spilimbergo"] },
  { sedi: ["Maniago"], blu: ["Claut"] },
  { sedi: ["Spilimbergo", "Meduno"], blu: ["Anduins"], sedi_liv: { Spilimbergo: 2, Meduno: 3 }, blu_liv: { Anduins: 4 } },
...
for (const [anno, mese] of MESI) {
  for (const amb of AMBITI) {
    for (const spec of SPEC) {
      ...
      s.test(`${anno}-${mese + 1} ${J(amb.ambito)} t=${J(amb.turni)}${amb.escludi ? " x" + J(amb.escludi) : ""} sedi=${J(spec.sedi || [])}${spec.blu ? " blu=" + J(spec.blu) : ""}`, () => {
...
s.test("agosto: dispo_set feriali/N ≡ dispo_aggiungi sui GIORNI_FERIALI_SEMPLICI (lista hardcodata)", () => {
  const a = { az: "dispo_set", medico: "X", ambito: "feriali", turni: ["N"], sedi: ["Maniago"], blu: ["Claut"] };
...
// --- Contenuto dell'entry: livelli e blu finiscono corretti negli slot ---
s.test("contenuto entry: sedi_liv/blu_liv riportati su ogni slot", () => {
  const a = { az: "dispo_set", medico: "X", ambito: { da: 3, a: 3 }, turni: ["N"], sedi: ["Spilimbergo", "Meduno"], blu: ["Anduins"], sedi_liv: { Spilimbergo: 2, Meduno: 3 }, blu_liv: { Anduins: 4 } };
  ...
  s.eq(J(only), J({ verde: ["Spilimbergo", "Meduno"], verdeLiv: { Spilimbergo: 2, Meduno: 3 }, blu: ["Anduins"], bluLiv: { Anduins: 4 }, no: false }), "entry non corrisponde alla spec");
});
...
s.test("no aliasing: gli slot sono copie indipendenti", () => {
  const a = { az: "dispo_set", medico: "X", ambito: "weekend", turni: ["G", "N"], sedi: ["Maniago", "Spilimbergo"] };
